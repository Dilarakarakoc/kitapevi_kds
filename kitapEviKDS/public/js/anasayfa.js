window.onload = function () {
    ...
    fetch('/api/sales-expenses')
        ...
        .then(data => {
            ...
            const salesData = data.map(item => item.toplam_gelir); 
            const expensesData = data.map(item => item.masraf_toplam); 
                    ...
                    datasets: [
                        {
                            label: 'Toplam Gelir',
                            data: salesData,
                            backgroundColor: 'rgba(167, 54, 227, 0.5)',  
                            borderColor: 'rgba(63, 16, 88, 0.5)',
                            borderWidth: 1,
                            yAxisID: 'y1'
                        },
                        {
    ...
    fetch('/api/total-sales')
        ...
        .then(data => {
            const labels = data.map(item => item.sube_ad); 
            const salesData = data.map(item => item.toplam_satis); 
...
const populationMap = L.map('population-map').setView([38.4237, 27.1428], 10); 
